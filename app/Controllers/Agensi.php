...
class Agensi extends BaseController
{
    ...
    public function index()
    {
        ...
        // 2.1 Dapatkan bulan dan tahun TERKINI (status_hantar = 0@1)
        $latestDateDraf = $db->table('table_report r')
            ->select('r.bulan, r.tahun, r.status_hantar')
            // Tambahan: Label status secara dinamik
            ->select("(CASE WHEN r.status_hantar IN (0,1) THEN 'Belum Dihantar' ELSE 'Dihantar' END) as status_label")
            ->join('table_quarters_profile tqp', 'tqp.id_kuarters = r.id_kuarters')
            ->where('tqp.id_agensi_induk', $id_agensi)
            ->whereIn('r.status_hantar', [0, 1])
            ->orderBy('r.tahun', 'DESC')
            ->orderBy('r.bulan', 'DESC')
            ->limit(1)
            ->get()
   ...
   if (!empty($reports)) {
    ...
    public function agensi_statistik_papar_excel($id_agensi, $bulan, $tahun)
    {
        ...
        foreach ($data as $row) {
            ...
            if ($status_dihuni == 'FALSE') {
                $sheet->getStyle('I' . $rowNum)->getFont()->getColor()->setARGB('FF0000');
            }
            if ($status_cadangan == 'FALSE') {
                $sheet->getStyle('K' . $rowNum)->getFont()->getColor()->setARGB('FF0000');
            }
            if ($status_total == 'FALSE') {
                $sheet->getStyle('AF' . $rowNum)->getFont()->getColor()->setARGB('FF0000');
            }

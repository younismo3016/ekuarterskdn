...
class Model_Rayuan_Baru extends Model
{
	...
	public function update_markah($data2, $id_surat_masuk)
    {
        $this->db->table('tbl_rayuan')->update($data2, array('id_surat_masuk' => $id_surat_masuk));
    }
    public function update_tarikh_perakuan($data, $no_kertas_perakuan)
    {
        $this->db->table('tbl_rayuan')->update($data, array('no_kertas_perakuan' => $no_kertas_perakuan));
    }
    ...
    function carian_surat_perakuan ($nama_pemohon=null,$no_ruj_jim=null,$rayuan_baru_ids=[])
    {
        ...
        cttn_kepakaran3,cttn_kepakaran4,pembatalan,catatan_batal,id_add,nama_pemohon,status_rayuan,cat_pas,
    ...
    function carian_selesai_perakuan ($no_kertas_perakuan=null)
    {

        $builder = $this->table('tbl_rayuan');
        $builder->select ('id_rayuan,umur,tarikh_perakuan,tbl_surat_masuk.id_surat_masuk,no_ruj_kdn,jim_cantum,id_jenis_pas,
        id_kategori_pemohon,umur,kel_akademik,catatan_akademik,kem_bahasa,tem_menetap,cttn_tem_menetap,
        tem_kahwin,hub_keluarga1,hub_keluarga2,hub_keluarga3,hub_keluarga4,hub_keluarga5
        hub_keluarga6,hub_keluarga7,hub_keluarga8,hub_keluarga9,cttn_hub_keluarga,
        tem_pengalaman,cttn_tem_pengalaman,nilai_pelaburan1,nilai_pelaburan2,nilai_pelaburan3,
        ...
        tarikh_ksu,tarikh_menteri,tarikh_penyemak,tarikh_pengesah');
        $builder->join('tbl_surat_masuk', 'tbl_rayuan.id_surat_masuk = tbl_surat_masuk.id_surat_masuk', 'left');
        $builder->limit(100);
        $builder->orderBy('id_surat_masuk desc');
        ...
        $builder->like('no_kertas_perakuan', $no_kertas_perakuan);
    } else {
        // Default to '1' if not provided
        $builder->like('no_kertas_perakuan', "X");
    }
        // $builder->orderBy('nama_pemohon', 'ASC');
        //dah pakai paginate tak payah pakai get()->getResult() dah
        return $builder->get()->getResultArray();
    ...
    function carian_surat_rayuan ($nama_pemohon=null,$id_kategori_pemohon=null,$rayuan_baru_ids=[])
    {
        ...
        cttn_kepakaran3,cttn_kepakaran4,pembatalan,catatan_batal,id_add,nama_pemohon,jantina,status_rayuan,
        status_perakuan,no_fail_jim,no_ruj_kdn,agama,warganegara,umur_real,tem_menetap_real,tem_kahwin_real,nama_penaja,
        no_kp_penaja,jumlah_markah,disemak_oleh,disahkan_oleh,ksu,yb_menteri,no_kertas_perakuan,cat_pas,keputusan');
        $builder->join('tbl_surat_masuk', 'tbl_rayuan.id_surat_masuk = tbl_surat_masuk.id_surat_masuk', 'right');
        $builder->whereIn('status_rayuan', ['2', '3']);
		$builder->whereNotIn('keputusan', ['GAGAL']);
        $builder->limit(100);
        $builder->orderBy('id_surat_masuk desc');
    ...
    function carian_kiraan_mata ($nama_pemohon=null,$no_ruj_jim=null,$rayuan_baru_ids=[])
    {
        ...
        $builder->join('tbl_surat_masuk', 'tbl_rayuan.id_surat_masuk = tbl_surat_masuk.id_surat_masuk', 'right');
		$builder->where('kategori_surat','1');
        $builder->limit(100);
        $builder->orderBy('id_surat_masuk desc');
    ...
    function carian_syor_penilai ($nama_pemohon=null,$no_ruj_jim=null,$rayuan_baru_ids=[])
    {
        ...
        nilai_pelaburan4,kepakaran1,kepakaran2,kepakaran3,kepakaran4,cttn_kepakaran1,cttn_kepakaran2,
        cttn_kepakaran3,cttn_kepakaran4,pembatalan,catatan_batal,id_add,nama_pemohon,status_rayuan,cat_pas,
        keputusan,no_fail_jim,umur_real,mar_umur,tem_menetap_real,tem_kahwin_real,tem_pengalaman_real');
        $builder->join('tbl_surat_masuk', 'tbl_rayuan.id_surat_masuk = tbl_surat_masuk.id_surat_masuk', 'right');
        $builder->whereIn('status_rayuan', ['1', '2']);
		$builder->whereNotIn('id_kategori_pemohon', ['']);
        $builder->limit(100);
        $builder->orderBy('id_surat_masuk desc');
